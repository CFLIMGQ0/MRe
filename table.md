...
## 当前五数据集模型与多模态基线汇总（2026-09-21）

下表用于当前阶段的总体比较，指标为五折验证集 Harrell C-index 的均值 ± 五折
标准差（各结果沿用其原始汇总的 `ddof` 口径）。需要特别注意实验批次边界：

- BLCA、BRCA 的 MRePath/PIBD/SurvPath 使用 2026-09 的患者互斥五折；
  COADREAD、STAD、HNSC 使用此前已经完成的仓库固定五折。
- `M3` 指定为 `BK_I03_mask2spline_hyperkan`。COREAD/STAD 数值来自历史
  Bernoulli Views + KAN 十配置实验，BLCA/BRCA/HNSC 数值来自 2026-09 的
  新队列。
- `AES + HGNN` 是 AES `p=0.8,T=0.001` 后端替换为原生 HGNN 的五数据集结果。
- DIMAF、LD-CVAE、SlotSPE 使用各自仓库的原生输入协议；它们不等同于
  ResNet50 输入条件下的严格单变量消融。

| 模型 | BLCA | BRCA | COADREAD | STAD | HNSC |
|---|---:|---:|---:|---:|---:|
| **MRePath（我们的复现）** | 0.6765 ± 0.0601 | 0.7137 ± 0.0504 | **0.7265 ± 0.1177** | 0.6288 ± 0.0508 | **0.6317 ± 0.0522** |
| PIBD | 0.5823 ± 0.0272 | 0.7177 ± 0.0526 | 0.6964 ± 0.1066 | 0.5886 ± 0.0403 | 0.5938 ± 0.0293 |
| SurvPath | 0.5696 ± 0.0534 | 0.6480 ± 0.0888 | 0.7217 ± 0.1028 | **0.6612 ± 0.0746** | 0.5931 ± 0.0523 |
| **AES + HGNN** | **0.6926 ± 0.0661** | **0.7160 ± 0.0648** | 0.7069 ± 0.1338 | 0.6250 ± 0.0243 | 0.6202 ± 0.0375 |
| **M3：Bernoulli Views + KAN（BK_I03）** | **0.6991 ± 0.0706** | 0.7098 ± 0.0498 | **0.8032 ± 0.0968** | 0.6066 ± 0.0406 | 0.6382 ± 0.0412 |
| DIMAF | 0.6490 ± 0.0815 | 0.6795 ± 0.0706 | 0.6576 ± 0.0616 | 0.6193 ± 0.0710 | 0.5810 ± 0.0275 |
| LD-CVAE | 0.6386 ± 0.0384 | 0.7418 ± 0.0956 | **0.7682 ± 0.1063** | 0.6592 ± 0.0378 | 0.6362 ± 0.0310 |
| SlotSPE | 0.6821 ± 0.0605 | **0.7441 ± 0.0745** | **0.7786 ± 0.1150** | 0.6447 ± 0.0818 | 0.6386 ± 0.0381 |

补充：STAD 上 M3 的最佳变体不是 `BK_I03`，而是 `BK_I06_bernoulli_jensen_debiased`
（0.6420 ± 0.0471）；这里没有替换 M3 主行，以保持 M3 定义一致。
