...
def segment_slide(
    ...
    with wsi_object.getOpenSlide() as slide:
        geometry = select_patch_geometry(slide, patch_mode)
        ...
        with wsi_object.visWSI(vis_level=vis_level, line_thickness=100) as mask:
...
@contextmanager
def managed_feature_loader(dataset: Any, batch_size: int, workers: int, device: torch.device):
    """One pass per slide; close worker pipes before the slide is closed.

    Persistent workers provide no reuse across these per-slide loaders. Combined
    with pin_memory, PyTorch 2.12.1 also retains their Process objects via atexit,
    accumulating descriptors until exit. Do not enable persistent_workers here.
    """
    loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=workers,
        pin_memory=device.type == "cuda", collate_fn=collate_features,
        persistent_workers=False,
    )
    iterator = None
    try:
        iterator = iter(loader)
        yield loader, iterator
    finally:
        # DataLoader has no public close API. This idempotent hook is needed for
        # early exits too (e.g. model/HDF5 errors retaining iterator tracebacks).
...
def open_fd_count() -> int | None:
    """Best-effort Linux resource diagnostic; never fail a slide for telemetry."""
    try:
...
def extract_features(
    ...
    with openslide.OpenSlide(str(slide_path)) as slide:
        ...
        mode = "w"
        with managed_feature_loader(dataset, batch_size, workers, device) as (loader, batches):
            with torch.inference_mode():
                for batch_index, (images, coords) in enumerate(batches, start=1):
    ...
    # Publish only a complete, validated feature file. Keep the last good cache
    # intact if a recomputation fails; .partial is never considered completed.
    count, dimension = validate_features(temp_path)
    if count != expected_count or dimension != 1024:
        raise RuntimeError(
            f"Feature validation failed: got {count}x{dimension}, expected {expected_count}x1024"
        )
...
def main() -> int:
    ...
    for index, slide_path in enumerate(slides, start=1):
        ...
        try:
            ...
            row["fd_after"] = open_fd_count()
            row["elapsed_seconds"] = round(time.time() - started, 3)
            row["updated_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
            status_by_id[slide_id] = row
            atomic_write_csv(status_path, list(status_by_id.values()))
            if row.get("feature_status") != "already_completed" or row.get("error"):
                print(f"[resources] {slide_id}: fd={row['fd_before']}->{row['fd_after']}", flush=True)
