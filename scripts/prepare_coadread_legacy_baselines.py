...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pibd-repo", type=Path, default=cache / "PIBD")
    parser.add_argument("--study", default="tcga_coadread")
    parser.add_argument("--cohort", default="coadread")
    parser.add_argument("--minimum-cases", type=int, default=290)
    return parser.parse_args()


def build_table(
    project_dir: Path, study: str, cohort: str, minimum_cases: int
) -> pd.DataFrame:
    metadata_path = (
        project_dir / f"datasets_csv/metadata/{study}.csv"
    )
    rna_path = (
        project_dir
        / f"datasets_csv/raw_rna_data/combine/{cohort}/rna_clean.csv"
    )
    ...
    if table["case_id"].nunique() < minimum_cases:
        raise RuntimeError(
            f"Too few paired {study} cases after joining metadata and RNA: "
...
def install_for_repo(
    ...
    split_dir = repo / f"splits/5foldcv/{study}"
    split_dir.mkdir(parents=True, exist_ok=True)
    if csv_dir_name is not None:
        csv_dir = repo / csv_dir_name
        csv_dir.mkdir(parents=True, exist_ok=True)
        table.to_csv(csv_dir / f"{study}_all_clean.csv", index=False)

    source_splits = project_dir / f"splits/5folds/{study}"
    for fold in range(5):
        shutil.copyfile(
...
def main() -> None:
    args = parse_args()
    table = build_table(
        args.project_dir, args.study, args.cohort, args.minimum_cases
    )
    ...
    install_for_repo(table, args.project_dir, args.survpath_repo, None, args.study)
    install_for_repo(table, args.project_dir, args.cmta_repo, "csv", args.study)
    install_for_repo(
        table, args.project_dir, args.porpoise_repo, "datasets_csv", args.study
    )
    install_for_repo(table, args.project_dir, args.pibd_repo, None, args.study)
    print(
        f"Prepared {args.study} legacy baseline inputs: "
