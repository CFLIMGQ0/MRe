...
"${PYTHON_BIN}" "${PROJECT_DIR}/scripts/prepare_coadread_legacy_baselines.py" \
    --study "${STUDY}" --cohort "${COHORT}" \
    --minimum-cases "${MREPATH_BASELINE_MINIMUM_CASES:-290}"

if [[ "${MREPATH_SKIP_CMTA:-0}" != "1" ]]; then
    ...
    echo "[baseline] skipping cmta because MREPATH_SKIP_CMTA=1"
fi
...
if [[ "${MREPATH_RESUME_BASELINES:-0}" == "1" && -f "${RESULTS_ROOT}/.snn_clam.complete" ]]; then
    echo "[baseline] skipping completed snn_clam summary"
else
    ...
    touch "${RESULTS_ROOT}/.snn_clam.complete"
fi
