...
SOURCE_ROOT="${PROJECT_ROOT}/data/tcga_brca/raw_svs"
OUTPUT_ROOT="${PROJECT_ROOT}/data/tcga_brca/clam_20x_resnet50_paper_k9"
MANIFEST="${PROJECT_ROOT}/data/tcga_brca/gdc_manifest_tcga_brca_mrepath.tsv"
...
if [[ "${incomplete}" -ne 0 ]]; then
  echo "[error] ${incomplete} BRCA manifest files remain incomplete"
  exit 1
fi
...
if [[ "${failed}" -ne 0 ]]; then
  echo "[error] at least one BRCA preprocessing shard failed"
  exit 1
fi

"${PREPROCESS_PYTHON_BIN}" -u scripts/build_wsi_graphs.py \
  --h5-dir "${OUTPUT_ROOT}/h5_files" \
  --output-dir "${OUTPUT_ROOT}/graph_files" \
  --metadata-csv "${PROJECT_ROOT}/datasets_csv/metadata/tcga_brca.csv" \
  --radius 9 \
  --feature-dim 1024 \
  --spatial-space l2 \
...
echo "[complete] BRCA ResNet50 graphs and offline hypergraph cache are ready"
